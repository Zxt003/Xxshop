...
public class CheckoutController {
     ...
     */
    @GetMapping("{id}")
    public String toCheckout(@PathVariable Long id, Model model, HttpSession session){//去结算，勾选购物车后前往支付界面，带上附属的信息，如id和信息
        // 获取用户 ID
        ...
        if (user != null){
            cartGoods = cartService.getByUserId(user.getId());//获取到指定id里面的购物车所有信息，在有user的前提下
        }
        Double total = 0.0;
        int goodsCount = cartGoods.size();
        if (cartGoods.size() >0){
            for (int i = 0; i< cartGoods.size(); i++){
                total += cartGoods.get(i).getBuyCount() * cartGoods.get(i).getGoodsPrice();//计算总价
            }
        }
        // 父分类
        ...
        model.addAttribute("categoryTwos",categoryTwos);//二级分类
        model.addAttribute("expresses",expresses);//快递
        return "checkout";
    }
    ...
    public void goPay(Long orderId, HttpServletResponse response) throws IOException {
        ...
        System.out.println("aliayClient:"+alipayClient);
        System.out.println("alipayResquest:"+alipayRequest);
    ...
    public String returnUrl(HttpServletRequest request, HttpServletResponse response, HttpSession session)
        ...
        if(signVerified){
            ...
            order.setId(orderId);
            order.setStatus((byte) 0);//售出为0
            System.out.println("支付成功"+order.getStatus());
