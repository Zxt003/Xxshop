...
import com.jzh.xx.transaction.mapper.ShopOrderMapper;
...
public class GoodsServiceImpl implements GoodsService {
    ...
    public Map<String, Object> IndexInfoBySellerId(Long id) {
        ...
        Example example = new Example(Goods.class);//根据商品
        Example example1 = new Example(ShopOrder.class);//根据订单
        //example.createCriteria().andEqualTo("sellerId",id).andEqualTo("sellStatus",0);
        example1.createCriteria().andEqualTo("status","1").andEqualTo("sellerId",id);
        List<ShopOrder> shopOrders = shopOrderMapper.selectByExample(example1);
        //List<Goods> goods = goodsMapper.selectByExample(example);

        int soldCount = shopOrders.size();
...
//        }
        for (ShopOrder shopOrder:shopOrders){
            soldMoney +=shopOrder.getOrderPrice();
        }
